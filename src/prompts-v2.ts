 ...
 *
 * DEPRECATION WARNING: This module (prompts-v2) is superseded by prompts-v2-1.
 * Please use prompts-v2-1.ts for new projects, which includes:
...
console.warn(
  "prompts-v2 is deprecated and will be removed in a future version. " +
    "Please use prompts-v2-1 instead for enhanced performance and features. " +
...
export const GENERATOR_V2_PROMPT = `# Identity and Metadata
...
export const CURATOR_V2_PROMPT = `# Identity and Metadata
...
export const GENERATOR_MATH_PROMPT = `# Identity and Metadata
...
export const GENERATOR_CODE_PROMPT = `# Identity and Metadata
...
export class PromptManager {
   ...
   *     const prompt = manager.getGeneratorPrompt("math", "2.0");
  ...
  getGeneratorPrompt(domain?: string, version?: string): string {
    ...
    let prompt = PromptManager.PROMPTS.generator[promptKey];
    if (typeof prompt === "string" && prompt.startsWith("ace.")) {
      // Handle v1 and v2.1 prompt references
      const moduleParts = prompt.split(".");
      if (moduleParts.length > 2 && moduleParts[1] === "prompts_v2_1") {
        const { [moduleParts[moduleParts.length - 1]]: importedPrompt } =
          require("./prompts-v2-1");
        prompt = importedPrompt;
      } else {
        const { [prompt.split(".").pop()!]: importedPrompt } =
          require("./prompts");
        prompt = importedPrompt;
      }
    }
    ...
    // Track usage
    this._trackUsage(`generator-${promptKey}`);

    // Add current date if v2 prompt
    if (prompt && prompt.includes("{current_date}")) {
    ...
    if (!prompt) {
      throw new Error(
        `No generator prompt found for version ${version}, domain ${domain}`
      );
    }
  ...
  getReflectorPrompt(version?: string): string {
    version = version || this.defaultVersion;
    let prompt = PromptManager.PROMPTS.reflector[version];
    ...
    this._trackUsage(`reflector-${version}`);
  ...
  getCuratorPrompt(version?: string): string {
    version = version || this.defaultVersion;
    let prompt = PromptManager.PROMPTS.curator[version];
    ...
    this._trackUsage(`curator-${version}`);
...
export function validatePromptOutput(
  ...
  if (role === "generator") {
    const required = ["reasoning", "bullet_ids", "final_answer"];
    ...
    if ("confidence_scores" in data) {
      for (const score of Object.values(data.confidence_scores)) {
...
export const MIGRATION_GUIDE = `# Migrating from v1 to v2 Prompts
...
\`\`\`typescript
// Old (v1)
import { GENERATOR_PROMPT, REFLECTOR_PROMPT, CURATOR_PROMPT } from './prompts';

// New (v2)
import { PromptManager } from './prompts-v2';

const manager = new PromptManager("2.0");
const generatorPrompt = manager.getGeneratorPrompt();
const reflectorPrompt = manager.getReflectorPrompt();
const curatorPrompt = manager.getCuratorPrompt();
\`\`\`
...
const mathPrompt = manager.getGeneratorPrompt("math");

// Code-specific generator
const codePrompt = manager.getGeneratorPrompt("code");
\`\`\`

## Custom Prompts with v2 Structure

\`\`\`typescript
import { GENERATOR_V2_PROMPT } from './prompts-v2';

// Use v2 as template
const customPrompt = GENERATOR_V2_PROMPT.replace(
