...
export const PLAYBOOK_USAGE_INSTRUCTIONS = `**How to use these strategies:**
 ...
 *
 * @param playbook - Playbook instance with learned strategies
 * @returns Formatted text with playbook strategies and usage instructions.
 ...
 * const context = wrapPlaybookForExternalAgent(playbook);
 * const enhancedTask = `${task}\n\n${context}`;
 * ```
 */
export function wrapPlaybookForExternalAgent(playbook: Playbook): string {
...
export const GENERATOR_V2_1_PROMPT = `# Identity and Metadata
...
export const CURATOR_V2_1_PROMPT = `# ⚡ QUICK REFERENCE ⚡
...
/**
 * Mathematics-specific Generator prompt for rigorous problem solving
 */
export const GENERATOR_MATH_V2_1_PROMPT = `# ⚡ QUICK REFERENCE ⚡
...
/**
 * Code-specific Generator prompt for software development
 */
export const GENERATOR_CODE_V2_1_PROMPT = `# ⚡ QUICK REFERENCE ⚡
...
export class PromptManager {
   ...
   */
  getGeneratorPrompt(domain?: string, version?: string): string {
    version = version || this.defaultVersion;

    // Check for domain-specific variant
    let promptKey: string;
    if (domain && `${version}-${domain}` in PromptManager.PROMPTS.generator) {
      promptKey = `${version}-${domain}`;
    } else {
      promptKey = version;
    }

    const prompt = (PromptManager.PROMPTS.generator as any)[promptKey];

    // Handle legacy v1 references (not implemented in TS port yet)
    if (typeof prompt === 'string' && prompt.startsWith('ace.')) {
      throw new Error(`Legacy prompt references not yet implemented: ${prompt}`);
    }

    // Track usage
    this.trackUsage(`generator-${promptKey}`);

    // Add current date for v2+ prompts
    if (prompt && version.startsWith('2') && prompt.includes('{current_date}')) {
      const currentDate = new Date().toISOString().split('T')[0];
      return prompt.replace('{current_date}', currentDate);
    }

    if (!prompt) {
      throw new Error(`No generator prompt found for version ${version}`);
    }
  ...
  getReflectorPrompt(version?: string): string {
    version = version || this.defaultVersion;
    const prompt = (PromptManager.PROMPTS.reflector as any)[version];

    if (typeof prompt === 'string' && prompt.startsWith('ace.')) {
      throw new Error(`Legacy prompt references not yet implemented: ${prompt}`);
    }

    this.trackUsage(`reflector-${version}`);

    if (!prompt) {
      throw new Error(`No reflector prompt found for version ${version}`);
    }
   ...
   */
  getCuratorPrompt(version?: string): string {
    version = version || this.defaultVersion;
    const prompt = (PromptManager.PROMPTS.curator as any)[version];

    if (typeof prompt === 'string' && prompt.startsWith('ace.')) {
      throw new Error(`Legacy prompt references not yet implemented: ${prompt}`);
    }

    this.trackUsage(`curator-${version}`);

    if (!prompt) {
      throw new Error(`No curator prompt found for version ${version}`);
    }
  ...
  static listAvailableVersions(): Record<string, string[]> {
    return {
      generator: Object.keys(PromptManager.PROMPTS.generator),
      reflector: Object.keys(PromptManager.PROMPTS.reflector),
      curator: Object.keys(PromptManager.PROMPTS.curator),
    };
  }
   ...
   *
   * @param role - The role (generator, reflector, curator)
   * @param testInput - Input parameters for testing
   * @returns Dict mapping version to formatted prompt preview
   */
  compareVersions(role: string, testInput: Record<string, any>): Record<string, string> {
    const results: Record<string, string> = {};
    const prompts = (PromptManager.PROMPTS as any)[role] || {};
 ...
 */
export function validatePromptOutputV21(output: string, role: string): ValidationResult {
  const errors: string[] = [];
  ...
  try {
    ...
    return { is_valid: false, errors, metrics: {} };
  }
  ...
  if (role === 'generator') {
    ...
    if ('confidence_scores' in data) {
      for (const [bulletId, score] of Object.entries(data.confidence_scores)) {
        const numScore = score as number;
        if (numScore < 0 || numScore > 1) {
          errors.push(`Invalid confidence score for ${bulletId}: ${numScore}`);
        } else {
          metrics[`confidence_${bulletId}`] = numScore;
        }
      }
    }
    ...
    const required = ['reasoning', 'error_identification', 'bullet_tags'];
    ...
    // Validate tags
    for (const tag of data.bullet_tags || []) {
...
\`\`\`typescript
import { validatePromptOutputV21 } from './prompts-v2-1';

// Returns validation + quality metrics
const result = validatePromptOutputV21(output, 'generator');
console.log(\`Quality score: \${(result.metrics.overall_quality || 0) * 100}%\`);
\`\`\`
...
const results = manager.compareVersions('generator', {
  playbook: '...',
  question: '...',
  context: '...',
  reflection: '...'
...
export interface PromptComparison {
  ...
  v21_enhancements?: Record<string, any>;
 ...
 *
 * @param role - Which role to compare (generator, reflector, curator)
 * @returns Comparison metrics and statistics
 */
export function comparePromptVersions(role: string = 'generator'): PromptComparison {
  ...
  try {
    if (role === 'generator') {
      v20Prompt = manager.getGeneratorPrompt(undefined, '2.0');
      v21Prompt = manager.getGeneratorPrompt(undefined, '2.1');
    } else if (role === 'reflector') {
      v20Prompt = manager.getReflectorPrompt('2.0');
      v21Prompt = manager.getReflectorPrompt('2.1');
    } else if (role === 'curator') {
      v20Prompt = manager.getCuratorPrompt('2.0');
      v21Prompt = manager.getCuratorPrompt('2.1');
    }
  } catch {
    // Legacy prompts not available
    return comparisons;
  }
  ...
  const v21Features = {
    ...
    atomicity_mentions:
      (v21Prompt.match(/atomic/gi) || []).length + (v21Prompt.match(/ATOMIC/g) || []).length,
